...
def drop_grp_photo(event):
    ...
    face_encodings=face_recognition.face_encodings(group_image)    
    #GUI
    global count1,img2
...
def start_finding():
    ...
    for en in enc:
        ...
        fp.seek(t)
        if True in matches:
            fp.write(s[0:count]+'P '+s[count+2:len(s)])
        else:
            fp.write(s[0:count]+'AB'+s[count+2:len(s)])
...
#l = tk.Label(background_label,bg='#541690',height=HEIGHT, width=WIDTH)
#l.place(relx=0.5, rely=0.1, relwidth=0.75, relheight=0.6, anchor='n')

gtextb = tk.Text(background_label, bd=4,height=HEIGHT+50, width=WIDTH)
gtextb.place(relx=0.5, rely=0.1, relwidth=0.75, relheight=0.6, anchor='n')
#gtextb.place(relx=0.5, rely=0.1,relwidth=0.95, relheight=0.75, anchor='n')
...
button1 = tk.Button(frame, text="Mark", font=60)
...
refresh_button0 = tk.Button(background_label,image=icon,command=refresh_grp)
refresh_button0.place(relx=0.85, rely=0.11, anchor='n')
